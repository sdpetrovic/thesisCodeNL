...
newestFileTSI = dTSI(newestIndexTSI); % Get the data corresponding to the newest file
...
TruthTSI = VariableVectorTSI((find(VariableVectorTSI(:,1)==20)),7:13);
...
% axis([currentVariable(1) 90 0.05 0.1]) % Third zoom

% axis([currentVariable(1) currentVariable(end) 0.05 0.06]) % Second zoom

axis([currentVariable(1) currentVariable(end) 0.0 0.05]) % Fourth zoom
...
% axis([min(functionEvaluationsTSI) 500 0.05 0.09]) % Zoom
% 
